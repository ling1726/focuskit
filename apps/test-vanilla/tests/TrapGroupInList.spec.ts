...
test('should set tabindex="-1" on TrapGroup parent', async ({ page }) => {
  ...
  await focuskitPage.render(html` <div id="list">
...
test("should not move focus into TrapGroup with arrow keys", async ({
  page,
}) => {
  const focuskitPage = new FocusKitPage(page);
  await focuskitPage.goto();
...
test("should not move focus out TrapGroup with arrow keys", async ({
  page,
}) => {
  const focuskitPage = new FocusKitPage(page);
  await focuskitPage.goto();
...
test("should continue with List after Escape from TrapGroup", async ({
  page,
}) => {
  const focuskitPage = new FocusKitPage(page);
  await focuskitPage.goto();
